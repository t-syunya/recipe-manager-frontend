...
export async function GET(req: NextRequest) {
	...
	if (googleKey && googleCx) {
		...
		if (googleRes.ok) {
			...
			if (!googleData.error) {
				return new Response(
					JSON.stringify({ engine: "google", ...googleData }),
					{ status: 200 },
				);
			}
		}
	...
	if (!braveKey) {
		return new Response(JSON.stringify({ error: "No search API available" }), {
			status: 500,
		});
	}
	...
	return new Response(JSON.stringify({ engine: "brave", ...braveData }), {
