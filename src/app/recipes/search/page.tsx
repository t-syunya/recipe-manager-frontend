  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data.engine === "google") {
        ...
        setResults((data.web?.results || []).map((item: any) => ({
          title: item.title,
          url: item.url,
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} mb={2}>
        <TextField label="キーワード" value={keyword} onChange={e => setKeyword(e.target.value)} size="small" />
        <Button variant="contained" color="primary" onClick={handleSearch} sx={{ borderRadius: 2 }} disabled={loading || !keyword}>
          {loading ? "検索中..." : "Web検索"}
        </Button>
